<div class="grid grid-cols-1 gap-6 p-3 ">
    ...
    <div class="panel">
        ...
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">
                {{ $this->colorIndex ? 'ویرایش رنگ' : 'افزودن رنگ جدید' }}</h5>
        </div>
        <div class="mb-5">
            <form class="space-y-5" wire:submit.prevent="createColor">
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div class="flex flex-col">
                        <label for="colorName" class="mb-1">نام رنگ</label>
                        <input wire:model="colorName" id="colorName" type="text" placeholder="نام رنگ را وارد کنید"
                            class="form-input">
                    ...
                    <div class="flex flex-col">
                        <label for="colorPicker" class="mb-1">انتخاب رنگ</label>
                        <input wire:model="colorCode" type="text" class="form-input" data-coloris
                ...
                @if ($colorIndex)
                    <div class="flex gap-4">
                        <button type="button" wire:click="updateColor" class="btn btn-success !mt-6">ویرایش
                            رنگ</button>
                        <button type="button" wire:click="editCancel" class="btn btn-danger !mt-6">لغو</button>
                    </div>
                @else
                    <button type="submit" class="btn btn-primary !mt-6">ثبت رنگ</button>
                @endif
            </form>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            ...
            <a href="{{ route('admin.trashed.colors.list') }}" type="button" class="btn btn-danger">
                ...
                <div class="mt-3 table-responsive">
                                ...
                                <tr>
                                    ...
                                    <td class="border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                        <button type="button" x-tooltip="Edit"
                                            wire:click="editColor({{ $color->id }})" @click="scrollToTop()">
                                        ...
                                        <button type="button" x-tooltip="Delete"
                                            wire:click="$dispatch('deleteConfirm',{ color_id : {{ $color->id }} })">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                    ...
                    <div class="panel">
                        <div class="flex flex-col justify-center w-full">
                            {{ $colors->links('admin.layouts.admin_pagination') }}
                        </div>
                    </div>
                </div>
            </div>
