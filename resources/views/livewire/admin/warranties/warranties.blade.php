<div class="grid grid-cols-1 gap-6 p-3">
    ...
    <div class="panel">
        ...
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">
                {{ $this->warrantyIndex ? 'ویرایش گارانتی' : 'افزودن گارانتی جدید' }}</h5>
        </div>
        <div class="mb-5">
            <form class="space-y-5" wire:submit.prevent="createWarranty">
        ...
        ">
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div class="">
                            <label for="warrantyName">نام گارانتی
                                <span class=" text-danger">*</span>
                            </label>
                            <input wire:model="warrantyName" id="warrantyName" type="text"
                                placeholder="نام گارانتی را وارد کنید" class="form-input">
                        ...
                        <div class="">
                            <label for="warrantyCode">کد گارانتی
                                <span class=" text-danger">*</span>
                            </label>
                            <input wire:model="warrantyCode" id="warrantyCode" type="text"
                                placeholder="کد گارانتی را وارد کنید" class="form-input">
                            <p class="mt-1 text-danger">
                                @error('warrantyCode')
                                    {{ $message }}
                                @enderror
                        ...
                        <div class="">
                            <label for="warrantyDescription">توضیحات گارانتی</label>
                            <textarea wire:model="warrantyDescription" id="warrantyDescription" placeholder="توضیحات گارانتی را وارد کنید" class="form-input" rows="3"></textarea>
                            <p class="mt-1 text-danger">
                                @error('warrantyDescription')
                                    {{ $message }}
                                @enderror
                            </p>
                        </div>
                        <div class="">
                            <label for="image">عکس گارانتی</label>
                            <input id="image" type="file" wire:model="image" x-ref="imageInput"
                                x-on:change="
                        if (!$event.target.files || !$event.target.files.length) {
                            // اگر کاربر فایلی انتخاب نکرد یا ورودی پاک شد
                    ...
                    @if ($warrantyIndex)
                        <div class="flex gap-4">
                            <button type="button" wire:click="updateWarranty" class="btn btn-success !mt-6">ویرایش
                                گارانتی</button>
                            <button type="button" wire:click="editCancel" class="btn btn-danger !mt-6">لغو</button>
                        </div>
                    @else
                        <button type="submit" class="btn btn-primary !mt-6">ثبت گارانتی</button>
                    @endif
                </div>
            </form>
        </div>
    ...
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            ...
            <a href="{{ route('admin.trashed.Warranties.list') }}" type="button" class="btn btn-danger">
                                ...
                                <tr>
                                    ...
                                    <td class="border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                        <button type="button" x-tooltip="Edit"
                                            wire:click="editWarranty({{ $warranty->id }})" @click="scrollToTop()">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
