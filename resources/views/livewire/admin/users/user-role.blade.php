<div class="grid grid-cols-1 gap-6 p-3 ">
    ...
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">
                {{ $this->selectedPermissionId ? 'ویرایش نقش' : 'افزودن نقش جدید' }}</h5>
        </div>
        <div class="mb-5">
            <form class="space-y-5" wire:submit.prevent="{{ $this->selectedPermissionId ? 'updateRole' : 'createRole' }}">
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        <label for="name">نام نقش
                            <span class="text-danger">*</span>
                        </label>
                        <input wire:model="name" id="name" type="text" placeholder="نام نقش را وارد کنید"
                            class="form-input">
                ...
                <div class="flex gap-4">
                    @if ($selectedPermissionId)
                        <button type="button" wire:click="updateRole" class="btn btn-success !mt-6">ویرایش
                            نقش</button>
                        <button type="button" wire:click="editCancel" class="btn btn-danger !mt-6">لغو</button>
                    @else
                        <button type="submit" class="btn btn-primary !mt-6">ثبت نقش</button>
                    @endif
                </div>
            </form>
        </div>
    ...
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">لیست نقش‌ها</h5>
        </div>
        @if ($roles->isEmpty())
            <hr class="mb-4">
                ...
                <div class="mt-3 table-responsive">
                                ...
                                <tr>
                                    ...
                                    <td x-data="modal">
                                        ...
                                        <div class="flex items-center justify-center">
                                            <button type="button" class="btn btn-info"
                                                wire:click="selectedRole({{ $role->id }})" @click="toggle">
                                                مجوزها</button>
                                        </div>
                                            ...
                                            <div class="flex items-center justify-center min-h-screen px-4"
                                                ...
                                                <div x-show="open" x-transition x-transition.duration.300
                                                    ...
                                                    <div
                                                        ...
                                                        <h5 class="text-lg font-bold"> افزودن مجوز به نقش</h5>
                                                    ...
                                                    <div class="p-5">
                                                        ...
                                                        <div class="flex items-center justify-end mt-8">
                                                            ...
                                                            <button type="button"
                                                                ...
                                                                @click="toggle"
                                                                wire:click="save_role_permission">ثبت</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <td class="border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                        ...
                                        <button type="button" x-tooltip="حذف"
                                            wire:click="$dispatch('deleteConfirm',{ role_id : {{ $role->id }} })">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                    ...
                    <div class="panel">
                        <div class="flex flex-col justify-center w-full">
                            {{ $roles->links('admin.layouts.admin_pagination') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('deleteConfirm', (event) => {
                const role_id = event.role_id;
                hardDeleteRole(role_id);
            });
        });
    </script>
</div>
