<div class="grid grid-cols-1 gap-6 p-3 ">
    {{-- show trashed articleCategories --}}
    <div class="panel">
        {{-- include loading --}}
        @include('admin.layouts.loading')
        <div class="flex items-center justify-between mb-5">
            <div class="grid grid-cols-1 gap-5 ">
                <h5 class="text-lg font-semibold dark:text-white-light">لیست دسته‌بندی‌های حذف شده</h5>
            </div>
            {{-- search box --}}
            ...
            <a href="{{ route('admin.article.categories.list') }}" type="button" class="btn btn-info">
                            ...
                            @foreach ($articleCategories as $index => $articleCategory)
                                <tr>
                                    ...
                                    <td class="border-b border-[#ebedf2] p-3 text-center dark:border-[#191e3a]">
                                        <button type="button" x-tooltip="بازگردانی"
                                            wire:click="$dispatch('confirmRestore',{ articleCategory_id : {{ $articleCategory->id }} })">
...
<script>
    document.addEventListener('livewire:init', () => {
        ...
        Livewire.on('deleteConfirm', (event) => {
            ...
            hardDeleteArticleCategory(articleCategory_id);
        });

        Livewire.on('confirmRestore', (event) => {
            const articleCategory_id = event.articleCategory_id;
            restoreArticleCategory(articleCategory_id);
        });
    });
</script>
