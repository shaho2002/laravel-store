    ...
    <div class="panel">
        <div class="flex items-center justify-between mb-5">
            <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                <h5 class="text-lg font-semibold dark:text-white-light">لیست تنوع قیمت محصول</h5>
                {{-- search box --}}
            ...
            <a href="{{ route('admin.create.product.price', $product->id) }}" type="button" class="btn btn-success">
                ...
                <div class="mt-3 table-responsive">
                    <table>
                        ...
                        <tbody>
                            @foreach ($productPrices as $index => $productPrice)
                                <tr>
                                    <td>{{ $productPrices->firstItem() + $index }}</td>
                                    <td>{{ $productPrice->name }}</td>
                                    <td>
                                        @if ($productPrice->color)
                                            <div class="flex items-center gap-2">
                                                <span class="w-5 h-5 rounded"
                                                    style="background-color: {{ $productPrice->color->code }} ">
                                                </span>
                                                <span>{{ $productPrice->color->name }}</span>
                                            </div>
                                        @else
                                            —
                                        @endif
                                    ...
                                    <td>{{ $productPrice->count }}</td>
                                    <td>{{ $productPrice->max_sell }}</td>
                                    <td>{{ number_format($productPrice->price) }} تومان</td>
                                    @if ($productPrice->status === 'active')
                                        <td class="text-success">فعال</td>
                                    @elseif($productPrice->status === 'notActive')
                                        <td class="text-danger">غیرفعال</td>
                                    @endif
                                    <td class="p-3 text-center">
                                        <div class="flex items-center justify-center gap-1.5">
                                            <!-- Edit Button -->
                                            <a href="{{ route('admin.edit.product.price',$productPrice->id) }}" x-tooltip="ویرایش"
                                            ...
                                            <button type="button" x-tooltip="حذف"
                                                wire:click="$dispatch('deleteConfirm',{ productFeature_id : {{ $productPrice->id }} })"
                    ...
                    <div class="panel">
                        <div class="flex flex-col justify-center w-full">
                            {{ $productPrices->links('admin.layouts.admin_pagination') }}
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('livewire:init', () => {
        ...
        Livewire.on('deleteConfirm', (event) => {
            const productFeature_id = event.productFeature_id;
            hardDeleteProductFeature(productFeature_id);
        });
    });
</script>
